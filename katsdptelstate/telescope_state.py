...
class TelescopeState(object):
    ...
    Keys are arranged into a hierarchy, separated by dots. An instance of this
    class has a list of prefixes which it will search for the key during
    lookups. New keys are added using the first prefix in the list. For
    convenience, :meth:`namespace` can be used to construct a prefix list which
    matches typical C++ namespace semantics (search in the most derived
    namespace first, walking up the hierarchy).

    Care should be used when putting an attribute in multiple namespaces,
    adding the second attribute may shadow an existing one for some views,
    ...
    prefixes : list of str
        ...
        specified directly for advances cases, it is normally generated by
        :meth:`namespace` and :meth:`add_prefix`. Writes are made using the
    ...
    def namespace(self, name):
        ...
        This adds `name` and a dot to the first existing prefix, as a new
        prefix. For example, if the prefixes are currently ``'x.y.', 'x.', ''``
        then the returned view will have prefixes
        ``'x.y.name.', 'x.y.', 'x.', ''``.

        Additionally, if `name` itself contains dots, then each intermediate
        namespace is also added i.e. ``self.namespace('a.b')`` is equivalent
        to ``self.namespace('a').namespace('b')``.
    ...
    def add_prefix(self, name):
        """Obtain a view in which all lookups are prefixed by `name`.

        The `name` is added to all existing prefixes. For example, if the
        prefixes are currently ``'x.y.', 'x.', ''`` then the new view will have
        prefixes ``'x.y.name.', 'x.name.', 'name.'``. Thus,
        ``self.add_prefix(name).get(key)`` is equivalent to ``self.get(name +
        '.' + key)``.
        """
        suffix = name + '.'
        new_prefixes = [prefix + suffix for prefix in self._prefixes]
        return self.__class__(None, None, prefixes=new_prefixes, base=self)
